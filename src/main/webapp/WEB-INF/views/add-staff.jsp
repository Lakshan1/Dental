  ...
  <div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto p-6">
        ...
        <div class="max-w-2xl">
          <h2 class="text-2xl font-bold text-slate-800 mb-1">Add Staff</h2>
          <p class="text-sm text-slate-400 mb-6">Fill in the details to add a new staff member.</p>
          ...
          ============================================================= --%>
          <form action="${pageContext.request.contextPath}/staffs/add" method="post"
            ...
            <div>
              ...
              <input type="password" id="password" name="password" required minlength="8"
                     ...
                     placeholder="Minimum 8 characters: upper, lower, number, symbol"
                     class="w-full px-3 py-2.5 text-sm rounded-lg border border-gray-300 focus:border-blue-600 focus:ring-1 focus:ring-blue-600 outline-none"/>
              <p class="text-xs text-slate-400 mt-1">At least 8 characters, including an uppercase letter, a lowercase letter, a number, and a special character. Will be hashed before saving.</p>
              <c:if test="${not empty passwordError}">
                <p class="text-xs text-rose-600 mt-1">${passwordError}</p>
              </c:if>
            </div>

            <%-- Role and status aren't asked here - every staff member created
                 via this form is role="staff", status="active" by default.
                 Change status later from the staff list if needed (e.g. On Leave). --%>
